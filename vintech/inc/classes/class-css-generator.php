...
class Vintech_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = vintech()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'vintech_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'vintech_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->vintech_generate_file_options();
		} );
	}

	function vintech_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->vintech_generate_file_options();
			$this->vintech_generate_file();
		}
	}

    function vintech_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->vintech_options_output() )
        ) );
    }

	function vintech_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function vintech_options_output() {
		$theme_colors                    = vintech_configs('theme_colors');
        $links                           = vintech_configs('link');
        $gradients                       = vintech_configs('gradient');
        $gradient_color_two        = vintech_configs('gradient_two');
		ob_start();

		printf('$vintech_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$vintech_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
    ...
    /* Inline CSS */
    function vintech_enqueue() {
        $css = $this->vintech_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function vintech_inline_css() {
        ob_start();
