 ...
 * Google Fonts
*/
function vintech_fonts_url() {
    $fonts_url = '';
    $fonts     = array();
    $subsets   = 'latin,latin-ext';
    
    if ( 'off' !== _x( 'on', 'Rubik font: on or off', 'vintech' ) ) {
        $fonts[] = 'Rubik:ital,wght@0,300..900;1,300..900&display=swap';
    }

    if ( 'off' !== _x( 'on', 'Inter font: on or off', 'vintech' ) ) {
 ...
 * Get page ID by Slug
*/
function vintech_get_id_by_slug($slug, $post_type){
 ...
 **/
function vintech_content_by_slug($slug, $post_type){
    $content = vintech_get_content_by_slug($slug, $post_type);

    $id = vintech_get_id_by_slug($slug, $post_type);
 ...
 * Get content by slug
 **/
function vintech_get_content_by_slug($slug, $post_type){
    $content = get_posts(
        array(
            'name'      => $slug,
            'post_type' => $post_type
 ...
 * Custom Comment List
 */
function vintech_comment_list( $comment, $args, $depth ) {
    if ( 'div' === $args['style'] ) {
        $tag       = 'div';
        $add_below = 'comment';
    } else {
 ...
 * Paginate Links
 */
function vintech_ajax_paginate_links($link){
 ...
 * RGB Color
 */
function vintech_hex_rgb($color) {
 ...
 * Image Size Crop
 */
if(!function_exists('vintech_get_image_by_size')){
    function vintech_get_image_by_size( $params = array() ) {
 ...
 * Search Form
 */
function vintech_header_mobile_search_form() { 
    $search_mobile = vintech()->get_theme_opt( 'search_mobile', false );
    $search_placeholder_mobile = vintech()->get_theme_opt( 'search_placeholder_mobile' );
    if($search_mobile) : ?>
        <div class="pxl-header-mobile-search pxl-hide-xl">
            <form role="search" method="get" action="<?php echo esc_url(home_url( '/' )); ?>">
                <input type="text" placeholder="<?php if(!empty($search_placeholder_mobile)) { echo esc_attr($search_placeholder_mobile); } else { esc_attr_e('Search...', 'vintech'); } ?>" name="s" class="search-field" />
...
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_year_shortcode() {
        ob_start(); ?>
        <span><?php the_date('Y'); ?></span>
        <?php $output = ob_get_clean();
        return $output;
    }
    pxl_register_shortcode('pxl_year', 'vintech_year_shortcode');
}

/* Highlight Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_text_highlight_shortcode( $atts = array() ) {
    ...
    pxl_register_shortcode('highlight', 'vintech_text_highlight_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_text_highlight_shortcode_editor( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_highlight', 'vintech_text_highlight_shortcode_editor');
}

/* Typewriter Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_text_typewriter_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
       ), $atts));
    ...
    pxl_register_shortcode('typewriter', 'vintech_text_typewriter_shortcode');
}

/* Button Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_btn_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
           'style' => '',
           'icon_class' => '',
    ...
    pxl_register_shortcode('pxl_button', 'vintech_btn_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_btn_video_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
           'url' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_btn_video', 'vintech_btn_video_shortcode');
}


if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_btn_submit_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'text' => '',
    ...
    pxl_register_shortcode('pxl_button_submit', 'vintech_btn_submit_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_slider_arrow( $atts = array() ) {
        extract(shortcode_atts(array(
           'type' => 'next',
           'style' => 'style-1',
       ), $atts));
...
pxl_register_shortcode('slider_arrow', 'vintech_slider_arrow');
}

// Shortcode Row/Column Grid
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_start_row_shortcode( $atts = array() ) {
        ob_start(); ?>
        <div class="pxl-post-container">
            ...
            pxl_register_shortcode('pxl_start_row', 'vintech_start_row_shortcode');
        }

        if(function_exists( 'pxl_register_shortcode' )) {
            function vintech_end_row_shortcode( $atts = array() ) {
                ob_start(); ?>
            </div>
        </div>       
        <?php $output = ob_get_clean();
    ...
    pxl_register_shortcode('pxl_end_row', 'vintech_end_row_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_start_col_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'class' => 'col-12',
       ), $atts));
       ob_start(); ?>
    ...
    pxl_register_shortcode('pxl_start_column', 'vintech_start_col_shortcode');
}

if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_end_col_shortcode( $atts = array() ) {
        ob_start(); ?>
    </div>  
    <?php $output = ob_get_clean();
    return $output;
}
pxl_register_shortcode('pxl_end_column', 'vintech_end_col_shortcode');
}
...
/* Gallery Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_gallery_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'link_video' => '',
           'images_id' => '',
           'col' => '2',
            ...
            foreach ($pxl_images as $key => $img_id) :
                ...
                <div class="pxl--item">
                    <div class="pxl--item-inner">
                        <a href="<?php echo esc_url($thumbnail_url); ?>" data-elementor-lightbox-slideshow="pxl-gallery-<?php echo esc_attr($pxl_g_id); ?>"><?php echo vintech_html($thumbnail); ?></a>
    ...
    pxl_register_shortcode('pxl_gallery', 'vintech_gallery_shortcode');
}

/* Addd shortcode Video button */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_video_button_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'link' => '',
           'text' => '',
           'class' => 'style1',
    ...
    pxl_register_shortcode('pxl_video_button', 'vintech_video_button_shortcode');
}

/* Get Category Shortcode  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_post_category_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'items' => '6',
           'columns' => '2',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_post_category', 'vintech_post_category_shortcode');
}

/* Slider 1  */
if(function_exists( 'pxl_register_shortcode' )) {
    function vintech_slider_price_shortcode( $atts = array() ) {
        extract(shortcode_atts(array(
           'price' => '',
           'desc' => '',
       ), $atts));
    ...
    pxl_register_shortcode('pxl_slider_price', 'vintech_slider_price_shortcode');
}

/**
 * Custom Widget Categories - Count
 */
add_filter('wp_list_categories', 'vintech_wg_category_count');
function vintech_wg_category_count($output) {
 ...
 */
add_filter('get_archives_link', 'vintech_wg_archive_count');
function vintech_wg_archive_count($links) {
 ...
 */
add_filter('wp_list_categories', 'vintech_wc_cat_count_span');
function vintech_wc_cat_count_span($links) {
    $dir = is_rtl() ? 'pxl-left' : 'pxl-right';
 ...
 */
function vintech_get_mega_menu_builder_id(){
    $mn_id = [];
 ...
 */
function vintech_get_page_popup_builder_id(){
    $pp_id = [];
...
function vintech_mouse_move_animation() { 
    $mouse_move_animation = vintech()->get_theme_opt('mouse_move_animation', false); 
    if($mouse_move_animation) {
        wp_enqueue_script( 'vintech-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array( 'jquery' ), '1.0.0', true ); ?>  
 ...
 * Start - Cookie Policy
 */
function vintech_cookie_policy() {
    $cookie_policy = vintech()->get_theme_opt('cookie_policy', 'hide');
    $cookie_policy_description = vintech()->get_theme_opt('cookie_policy_description');
    $cookie_policy_btntext = vintech()->get_theme_opt('cookie_policy_btntext');
    $cookie_policy_link = get_permalink(vintech()->get_theme_opt('cookie_policy_link')); 
 ...
 */
function vintech_subscribe_popup() {
    $subscribe = vintech()->get_theme_opt('subscribe', 'hide');
    $subscribe_layout = vintech()->get_theme_opt('subscribe_layout');
    $popup_effect = vintech()->get_theme_opt('popup_effect', 'fade');
    $args = [
 ...
 */
add_action( 'show_user_profile', 'vintech_user_fields' );
add_action( 'edit_user_profile', 'vintech_user_fields' );
function vintech_user_fields($user){
    ...
    ?>
    <h3><?php esc_html_e('Theme Custom', 'vintech'); ?></h3>
    <table class="form-table">
        <tr>
            <th><label for="user_name"><?php esc_html_e('Author Name', 'vintech'); ?></label></th>
            <td>
        ...
        <tr>
            <th><label for="user_position"><?php esc_html_e('Author Position', 'vintech'); ?></label></th>
            <td>
...
function vintech_get_user_name() {
...
function vintech_get_user_position() {
...
function vintech_attachment_field_credit( $form_fields, $post ) {
...
add_filter( 'attachment_fields_to_edit', 'vintech_attachment_field_credit', 1, 2 );


function vintech_attachment_field_credit_save( $post, $attachment ) {
...
add_filter( 'attachment_fields_to_save', 'vintech_attachment_field_credit_save', 1, 2 );
/* End Custom Field Media */

/* Author Social */
function vintech_get_user_social() {
