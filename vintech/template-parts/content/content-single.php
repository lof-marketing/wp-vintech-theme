...
$sg_featured_img_size = vintech()->get_theme_opt('sg_featured_img_size', '1080x354');
$feature_image_display = vintech()->get_theme_opt('feature_image_display', 'hide');
$post_breadcrumb_on = vintech()->get_opt( 'post_breadcrumb_on', '1' ); 
?>
    ...
    <div class="<?php echo esc_attr($content_inner_cls);?>">
        <?php if($post_breadcrumb_on == '1') : ?>
            <div class="pxl-post--top">
                <?php if($post_breadcrumb_on == '1') : ?>
                    <?php vintech()->page->get_breadcrumb(); ?>
                <?php endif; ?>
                <?php vintech()->blog->get_post_title(); ?>     
                <?php
                vintech()->blog->get_post_metas();
                if (has_post_thumbnail()) {
...
if ($post_tag == '1' || $post_social_share == '1'){
    ...
    <div class="post-tags-share d-flex">
        <?php
        if ($post_tag == '1'){
            ?><div class="post-tags-wrap "><?php  vintech()->blog->get_post_tags(); ?></div><?php
        }
